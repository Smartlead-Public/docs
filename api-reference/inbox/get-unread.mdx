...
description: "Fetch unread replies from leads with filtering by campaign, status, and categories"
---
...
```bash cURL
curl -X POST "https://server.smartlead.ai/api/v1/master-inbox/unread-replies?api_key=YOUR_KEY" \
...
const response = await fetch(
  `https://server.smartlead.ai/api/v1/master-inbox/unread-replies?api_key=${API_KEY}`,
  {
    method: 'POST',
