...
<Note>
  Both `forward_email_body` and `forward_email_subject` are optional. Existing integrations that send only `message_id`, `stats_id`, and `to_emails` are fully backwards-compatible — behaviour is unchanged when these fields are omitted.
</Note>
