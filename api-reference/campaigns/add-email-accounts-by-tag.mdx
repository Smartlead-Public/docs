...
<ParamField query="max_limit" type="boolean" default="false">
  Raises the maximum number of mailboxes a single campaign may hold from **2,500** to **5,000**. It does not change how many tags or mailboxes this call may reference - it only raises the ceiling the campaign is checked against.

  Same flag, same meaning, as on [Add Email Accounts](/api-reference/campaigns/add-email-accounts). If the campaign is already at the cap the request returns `400`; if the tag resolves to more mailboxes than the remaining headroom, the extras are not added and the response `message` says how many were left out.
</ParamField>
