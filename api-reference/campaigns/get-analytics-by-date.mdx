...
<ResponseField name="human_reply_count" type="string">
  Replies in the date range from leads that are **not** categorised as out-of-office. See [Human vs total replies](#human-vs-total-replies).
</ResponseField>
...
|---|---|
| `total_reply_count` | Every reply, out-of-office included |
| `human_reply_count` | Replies from leads not categorised as out-of-office |

Out-of-office replies are the difference between the two:

```
ooo_replies = total_reply_count - human_reply_count
```

Both fields are derived from SmartLead's reply categorisation, so they do **not** depend on the per-campaign *ignore out-of-office* setting. You get the split whether or not that toggle is enabled.

If reply categorisation has not run for a campaign, leads stay uncategorised and `human_reply_count` equals `total_reply_count`.
...
print(f"Out-of-office:  {total - human}")
```
...
console.log(`Out-of-office:  ${total - human}`);
```
