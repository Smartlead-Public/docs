...
- `reply_count` is **5**, but 3 of those are auto-replies. That is the inflation customers complain about.
- `non_ooo_reply_count` is **3** — the replies actually worth looking at.
- Out-of-office replies are `6 - 3 = 3`.
- Row 6 is counted in `non_ooo_reply_count` but not in `reply_count`, because it was flagged as ignored for a reason that has nothing to do with out-of-office.
