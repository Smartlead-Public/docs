...
**Key Features**:
- Validates account ownership before adding

<Warning>
  This endpoint validates that you own each mailbox, but does **not** check connection health. A mailbox is added whether or not `is_smtp_success` and `is_imap_success` are true.
</Warning>

<Note>
  To attach mailboxes by tag instead of by ID, use [Add Email Accounts by Tag](/api-reference/campaigns/add-email-accounts-by-tag).
</Note>
