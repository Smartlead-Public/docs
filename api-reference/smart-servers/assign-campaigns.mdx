...
<Warning>
This endpoint only creates new assignments. A campaign that already belongs to another SmartInfra is rejected with `409`, and a campaign already attached to this SmartInfra is rejected with `208` — neither is silently overwritten. Move an existing assignment from the SmartInfra page in the dashboard.
</Warning>
